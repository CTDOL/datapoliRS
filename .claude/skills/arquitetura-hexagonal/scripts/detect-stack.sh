...
# -name (não -path) para podar em qualquer profundidade — monorepo com
# frontend/node_modules, por exemplo, não escapa da poda.
N=$(find . \( -name node_modules -o -name vendor -o -name .git -o -name .next \
              -o -name dist -o -name build -o -name .venv -o -name venv -o -name __pycache__ \) -prune -o \
...
# ---------- TAMANHO ----------
echo "TAMANHO"
CODE=$(find . \( -name node_modules -o -name vendor -o -name .git -o -name .next \
                  -o -name dist -o -name build -o -name .venv -o -name venv -o -name __pycache__ \) -prune -o \
...
# SQL concentrado em arquivos de "repositório" é a camada fazendo o trabalho dela,
# não um cheiro — só espalhado fora dela (controller, página, service) é sinal real.
SQL_FILES=$(grep -rlE "(SELECT|INSERT|UPDATE|DELETE)[[:space:]]" --include='*.php' --include='*.js' --include='*.ts' --include='*.py' \
      --exclude-dir=vendor --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=dist \
      --exclude-dir=build --exclude-dir=.venv --exclude-dir=venv --exclude-dir=__pycache__ . 2>/dev/null)
SQL=$(printf '%s\n' "$SQL_FILES" | grep -c .)
SQL_FORA=$(printf '%s\n' "$SQL_FILES" | grep -viE '(^|/)(repositor(y|io|ies)|repo)([_./]|$)' | grep -c .)
echo "  Arquivos com SQL embutido: $SQL"
if [ "$SQL_FORA" -gt 3 ]; then
  echo "    >> SQL espalhado fora de camada de repositório ($SQL_FORA de $SQL arquivo(s)) indica ausência de camada de repositório."
elif [ "$SQL" -gt 3 ]; then
  echo "    >> SQL concentrado em arquivos de repositório ($SQL total, $SQL_FORA fora dela) — sinal de camada de repositório presente, não de acoplamento espalhado. Confirme lendo o(s) arquivo(s) fora dela, se houver."
fi
...
CRED=$(grep -rlE "(password|senha|secret|api_key)[[:space:]]*=[[:space:]]*[\"'][^\"']{3,}" \
       --include='*.php' --include='*.js' --include='*.py' --exclude-dir=vendor --exclude-dir=node_modules \
       --exclude-dir=.next --exclude-dir=dist --exclude-dir=build --exclude-dir=.venv --exclude-dir=venv \
